...
@LazyInit private val TypeJvmDescriptor: WritableSlice<KotlinType, String> = Slices.createSimpleSlice()
...
fun KotlinType.toJvmDescriptor(trace: BindingTrace, forceObject: Boolean = false): String? {
  val descriptor = trace[TypeJvmDescriptor, this] ?: when {
    // Boolean | Byte | Char | Short | Int | Long | Float | Double
    KotlinBuiltIns.isPrimitiveType(this) -> when (fqNameUnsafe) {
      FqNames._boolean -> if (forceObject) "Ljava/lang/Boolean" else "Z"
      FqNames._byte -> if (forceObject) "Ljava/lang/Byte" else "B"
      FqNames._char -> if (forceObject) "Ljava/lang/Character" else "C"
      FqNames._short -> if (forceObject) "Ljava/lang/Short" else "S"
      FqNames._int -> if (forceObject) "Ljava/lang/Integer" else "I"
      FqNames._long -> if (forceObject) "Ljava/lang/Long" else "J"
      FqNames._float -> if (forceObject) "Ljava/lang/Float" else "F"
      FqNames._double -> if (forceObject) "Ljava/lang/Double" else "D"
      else -> null
    }
    ...
    KotlinBuiltIns.isArray(this) -> (arguments.singleOrNull() ?: return null).let {
      if (it.isStarProjection) "[java.lang.Object" else it.type.toJvmDescriptor(trace)?.run { "[$this" }
    }
  ...
  trace.record(TypeJvmDescriptor, this, descriptor)
